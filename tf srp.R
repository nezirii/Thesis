...
M0.1<-gls(srp ~ impact+f.time, 
        na.action=na.omit, data=tf, weights=vf1)

M0.2<-gls(srp ~ impact+f.time, 
          na.action=na.omit, data=tf, weights=vf2)

M0.3<-gls(srp ~ impact+f.time, 
          na.action=na.omit, data=tf, weights=vf3)

M0.4<-gls(srp ~ impact+f.time, 
          na.action=na.omit, data=tf, weights=vf4)

M0.5<-gls(srp ~ impact+f.time, 
          na.action=na.omit, data=tf, weights=vf5)

M0.6<-gls(srp ~ impact+f.time, 
          na.action=na.omit, data=tf, weights=vf6)
#No Convergence

M0.7<-gls(srp ~ impact+f.time, 
          na.action=na.omit, data=tf, weights=vf7)

M0.8<-gls(srp ~ impact+f.time, 
          na.action=na.omit, data=tf, weights=vf8)
#No Convergence

M0.9<-gls(srp ~ impact+f.time, 
          na.action=na.omit, data=tf, weights=vf9)

M0.10<-gls(srp ~ impact+f.time, 
          na.action=na.omit, data=tf, weights=vf10)
#No Connvergence

M0.11<-gls(srp ~ impact+f.time, 
          na.action=na.omit, data=tf, weights=vf11)

anova(M0,M0.1,M0.2,M0.3,M0.4,M0.5,M0.7,M0.9,M0.11)
#M1.10 is best with varIdent as a function of time

E0.7<-residuals(M0.7)

plot(filter(tf, !is.na(srp)) %>%dplyr::select(location),
     E0.7, xlab="Location", ylab="Residuals")
plot(filter(tf, !is.na(srp)) %>%dplyr::select(impact),
     E0.7, xlab="Location", ylab="Residuals")

qqnorm(residuals(M0.7))
qqline(residuals(M0.7))
ad.test(residuals(M0.7))

x<-tf$srp[!is.na(tf$srp)]#removes na values from column
E0.7<-residuals(M0.7,type="normalized")
plot(M0.7) #residuals vs fitted values
plot(x, E0.7)
...
x <- group_by(tf, impact, f.time) %>%  # Grouping function causes subsequent functions to aggregate by season and reach
  summarize(srp.mean = mean(srp, na.rm = TRUE), # na.rm = TRUE to remove missing values
            ...
            n = sum(!is.na(srp)), # of observations, excluding NAs. 
...
M1.14<-lme(srp ~ impact+f.time, random=~1|location, 
...
M1.15<-lme(srp ~ impact+f.time, random=~1|location, 
