...
M0<-gls(net.mineralization ~ impact+f.time, 
        na.action=na.omit, data=sm, method="ML")

#add random factor - refer to chapter 5 of zuur

M1<-lme(net.mineralization ~ impact+f.time, 
        random=~ 1 | location, na.action=na.omit, data=sm, method="ML")

#try nesting

M2<-lme(net.mineralization ~ impact+f.time, random=~1|nest, 
        na.action=na.omit, data=sm, method="ML")

#try interaction with random factor

M3<-lme(net.mineralization ~ impact*f.time, 
        random=~ 1 | location, na.action=na.omit, data=sm, method="ML")

M4<-lme(net.mineralization ~ impact*f.time, random=~1|nest, 
        na.action=na.omit, data=sm, method="ML")
...
plot(filter(sm, !is.na(net.mineralization)) %>%dplyr::select(location),
     E1, xlab="Location", ylab="Residuals")
plot(filter(sm, !is.na(net.mineralization)) %>%dplyr::select(impact),
     E1, xlab="Location", ylab="Residuals")
...
x<-sm$net.mineralization[!is.na(sm$net.mineralization)]#removes na values from column
...
x <- group_by(sm, time, impact) %>%  # Grouping function causes subsequent functions to aggregate by season and reach
  summarize(net.mineralization.mean = abs(mean(net.mineralization, na.rm = TRUE)), # na.rm = TRUE to remove missing values
            net.mineralization.sd=abs(sd(net.mineralization, na.rm = TRUE)),  # na.rm = TRUE to remove missing values
            n = sum(!is.na(net.mineralization)), # of observations, excluding NAs. 
            net.mineralization.se=net.mineralization.sd/sqrt(n))

ggplot(data=x, 
       aes(x=time, y=net.mineralization.mean, fill=impact)) + 
  geom_bar(stat="identity", position=position_dodge(), color = "black") + 
  geom_errorbar(aes(ymin=net.mineralization.mean, ymax=net.mineralization.mean+net.mineralization.se), width=0.2, 
...
M1.14<-lme(net.mineralization ~ impact+f.time, 
           random=~1 | location, na.action=na.omit, data=sm, weights=vf2, correlation=cs1)

M1.15<-lme(net.mineralization ~ impact+f.time, 
...
sm$log.net.mineralization<-log10(sm$net.mineralization)+1

M1<-lme(log.net.mineralization ~ impact+f.time, 
        random=~ 1 | location, na.action=na.omit, data=sm, method="ML")

#try nesting

M2<-lme(log.net.mineralization ~ impact+f.time, random=~1|nest, 
        na.action=na.omit, data=sm, method="ML")

#try interaction with random factor

M3<-lme(log.net.mineralization ~ impact*f.time, 
        random=~ 1 | location, na.action=na.omit, data=sm, method="ML")

M4<-lme(log.net.mineralization ~ impact*f.time, random=~1|nest, 
        na.action=na.omit, data=sm, method="ML")
...
plot(filter(sm, !is.na(log.net.mineralization)) %>%dplyr::select(location),
     E3, xlab="Location", ylab="Residuals")
plot(filter(sm, !is.na(log.net.mineralization)) %>%dplyr::select(impact),
     E3, xlab="Location", ylab="Residuals")
...
#####################################################
#final model
M.full<-lme(log.net.mineralization ~ impact*f.time, 
...
log.net.mineralization.emm = data.frame(cbind(xx,impact,event))
log.net.mineralization.emm$emmean.raw = (10^(log.net.mineralization.emm$emmean))
log.net.mineralization.emm$SE.raw = (10^(log.net.mineralization.emm$emmean))

#this is the final table you can use for plotting
log.net.mineralization.emm

x = log.net.mineralization.emm
